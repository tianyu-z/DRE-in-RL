...
def main():
    ...
    for n_epi in range(10000):
        ...
        if n_epi > 100: # train the lstm
            ...
            input_data = torch.vstack((tensor_data_old_train, tensor_data)).unsqueeze(0).double()
            input_label = torch.tensor(label).unsqueeze(-1).double()
            prediction = lstm(input_data)
            loss = loss_fn(prediction, input_label)
            loss.mean().backward()
            optimizer_lstm.step()
        if n_epi ==0:
            tensor_data_old = []
        ...
        if n_epi%print_interval==0 and n_epi!=0:
            ...
            if n_epi%100 == 0 and n_epi!=0:
                torch.save(save_dict,"ckpt_{}_lstm.pth".format(n_epi))
            score = 0.0
    env.close()
